...
function enginePicker() {
  return `<fieldset class="engine"><legend>내 게임의 ‘렌더링 엔진(Vulkan)’은?</legend><div class="engine-options"><label><input name="engine" type="radio" value="vulkan" ${engine==='vulkan'?'checked':''}>켜짐 · 체크되어 있음</label><label><input name="engine" type="radio" value="dx9" ${engine==='dx9'?'checked':''}>꺼짐 · 체크되어 있지 않음</label></div></fieldset>`;
}
const pages = [
  ...
  () => ({title:'게임에서 Home을 눌러 보세요',lead:'평소 방법으로 마비노기를 실행하고, 설치 성공 안내를 확인해요.',html:`${media('game-banner.png','게임 위쪽에 표시된 ReShade 설치 성공 안내와 Home 키 안내.',[],'게임 화면에서 안내 문구만 잘랐습니다. 캐릭터명과 채팅은 포함하지 않았어요.')}<ol class="instructions"><li>게임 화면 위쪽에 <strong>ReShade</strong>와 설치 성공 안내가 뜨는지 확인하세요.</li><li>게임 화면을 클릭한 뒤 키보드의 <kbd>Home</kbd>을 누르세요. 노트북은 <kbd>Fn</kbd>과 함께 눌러야 할 수도 있어요.</li><li>첫 사용 안내가 나오면 화면의 계속 버튼으로 진행하거나 튜토리얼을 건너뛰세요.</li><li>설정창이 열리면 설치 확인이 끝났어요. <kbd>Home</kbd>을 다시 누르면 닫힙니다.</li></ol><p class="note">설치 성공 안내가 없거나 Home을 눌러도 열리지 않으면 마지막 단계의 ‘설정창이 안 열려요’를 확인하세요.</p>`}),
...
pages[6]=()=>{const page=originalColorPage();page.html=media('vibrance-enabled.png','검색 결과의 Vibrance 체크가 켜진 실제 게임 설정 화면.',[],'실제 게임에서 활성화 상태를 확인했습니다.')+media('vibrance-value.png','Vibrance 값 0.100이 표시된 실제 게임 설정 화면.',[],'설정창의 효과 목록과 값 부분을 각각 잘라 보여 줍니다.')+page.html+`<details class="trouble"><summary>같은 설정 파일을 받아서 쓰고 싶어요</summary><p><a class="button secondary" href="Mabinogi-Soft.ini" download>약한 색감 설정 받기 (.ini)</a></p><ol><li>파일을 마비노기 설치 폴더에 넣으세요. 기존 파일을 덮어쓸 필요는 없어요.</li><li>ReShade 창 맨 위의 프리셋 선택 부분에서 <strong>Mabinogi-Soft.ini</strong>를 선택하세요.</li><li>Vibrance 체크와 값 <strong>0.100</strong>을 확인하세요. SweetFX가 먼저 설치되어 있어야 해요.</li></ol><p>이 파일에는 색감 수치만 들어 있습니다. ReShade나 효과 파일은 포함하지 않아요.</p></details>`;return page};
